...
const AetherUI = {
    ...
    _attr(el, name) {
        return el && el.getAttribute
            ? el.getAttribute(`${this._pfx}-${name}`)
            : null;
    },
    _has(el, name) {
        return el && el.hasAttribute
            ? el.hasAttribute(`${this._pfx}-${name}`)
            : false;
    },
    ...
    open(id, triggerEl = null) {
        ...
        [...this.openStack].reverse().forEach((openId) => {
            ...
            if (!openEl || (el && openEl.contains(el))) return;
    ...
    close(id) {
        ...
        this.openStack
            .filter((oid) => {
                ...
                return child && el && el.contains(child) && oid !== id;
            })
